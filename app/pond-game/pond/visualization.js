...
function draw() {
    ...
    for (const i in avatars) {
        ...
        drawAvatar(ctx, avatars[i], i === _highlightedAvatar);
    }
...
function drawAvatar(ctx, avatar, highlighted) {
    ...
    const scale = avatarSize * viewport.width;
    ctx.translate(x, y);
...
function drawMissile(ctx, missile) {
    ...
    ctx.beginPath();
    ctx.arc(missilePos.x, shadowPos.y, Math.max(0, 1 - parabola / 10) * missileRadius * viewport.width, 0, Math.PI * 2, true);
    ctx.closePath();
    ctx.fillStyle = 'rgba(128, 128, 128, ' + Math.max(0, 1 - parabola / 10) + ')';
    ctx.fill();
    ctx.beginPath();
    ctx.arc(missilePos.x, missilePos.y, missileRadius * viewport.width, 0, Math.PI * 2, true);
    ctx.closePath();
